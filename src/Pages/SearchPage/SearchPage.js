...
const SearchPage = () => {
  ...
  // Fetch cars initially on page load with filters
  useEffect(() => {
    fetchFilteredCars();
  }, [filters, currentPage]); // Trigger fetching on filter or page change
  ...
  const fetchFilteredCars = async () => {
    try {
      ...
      setTotalPages(Math.ceil(response.data.totalCars / carsPerPage)); // Calculate total pages based on totalCars
    } catch (error) {
      ...
      <div className="flex">
        ...
        <div className="w-1/4 p-4 bg-white shadow-md">
          ...
          <div>
            ...
            <button onClick={fetchFilteredCars} className="w-full p-2 mt-4 bg-blue-600 text-white rounded-lg">
              Search
            </button>
          </div>
        </div>
